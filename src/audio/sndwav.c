...
void wav_pause(WavPlayerId playerId) {
    ...
    if (streams[playerId].status == SNDDEC_STATUS_READY || streams[playerId].status == SNDDEC_STATUS_PAUSING)
        return;
...
void wav_stop(WavPlayerId playerId) {
    ...
    if (streams[playerId].status == SNDDEC_STATUS_READY || streams[playerId].status == SNDDEC_STATUS_STOPPING)
        return;
...
static void* audio_cb(snd_stream_hnd_t shnd, int req, int* done) {
    ...
    if (streams[playerId].loop) {
        ...
        if ((pos + req) >= total_len) {
            uint32_t over = (((pos + req) - total_len));
            uint32_t actual = (req - over);
            ...
            if (-1 == actually_read) {
                printf("actual begin: fs_read failed for player id %d playing %s (%s)\n", playerId,
                       streams[playerId].filename, strerror(errno));
            ...
            while (actually_read < actual) {
                ssize_t next_read_actual = fs_read(streams[playerId].wave_file,
                                                   streams[playerId].drv_buf + actually_read, actual - actually_read);
            ...
            if (new_offs != streams[playerId].loop_start) {
                printf("looping: fs_seek failed for player id %d playing %s (%s)\n", playerId,
                       streams[playerId].filename, strerror(errno));
                printf("this player will be stopped\n");
                snd_stream_stop(streams[playerId].shnd);
                *done = 0;
            ...
            while (actually_read < over) {
                ssize_t next_read_actual =
                    fs_read(streams[playerId].wave_file, streams[playerId].drv_buf + actual + actually_read,
                            over - actually_read);
                if (-1 == next_read_actual) {
            ...
            while (actually_read < req) {
                ssize_t next_read_actual = fs_read(streams[playerId].wave_file,
                                                   streams[playerId].drv_buf + actually_read, req - actually_read);
                if (-1 == next_read_actual) {
        ...
        *done = req;
        ...
        if (actually_read != req) {
            ...
            if (new_offs != streams[playerId].loop_start) {
                printf("onetime: fs_seek failed for player id %d playing %s (%s)\n", playerId,
                       streams[playerId].filename, strerror(errno));
                printf("this player will be stopped\n");
                snd_stream_stop(streams[playerId].shnd);
                *done = 0;
